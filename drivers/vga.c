...
/* А..Я, 5x7. Lower-case Cyrillic is intentionally rendered as upper-case for the HUD font. */
static const u8 cyr[32][7]={
 {14,17,17,31,17,17,17}, /* А */
 {31,16,16,30,17,17,30}, /* Б */
 {30,17,17,30,17,17,30}, /* В */
 {31,16,16,16,16,16,16}, /* Г */
 {14,10,10,10,31,17,17}, /* Д */
 {31,16,16,30,16,16,31}, /* Е */
 {21,21,14,4,14,21,21},  /* Ж */
 {30,1,1,14,1,1,30},     /* З */
 {17,19,21,25,17,17,17}, /* И */
 {10,4,17,19,21,25,17},  /* Й */
 {17,18,20,24,20,18,17}, /* К */
 {7,9,17,17,17,17,17},   /* Л */
 {17,27,21,21,17,17,17}, /* М */
 {17,17,17,31,17,17,17}, /* Н */
 {14,17,17,17,17,17,14}, /* О */
 {31,17,17,17,17,17,17}, /* П */
 {30,17,17,30,16,16,16}, /* Р */
 {14,17,16,16,16,17,14}, /* С */
 {31,4,4,4,4,4,4},       /* Т */
 {17,17,10,4,4,8,16},    /* У */
 {4,14,21,21,14,4,4},    /* Ф */
 {17,17,10,4,10,17,17},  /* Х */
 {17,17,17,17,17,31,1},  /* Ц */
 {17,17,17,15,1,1,1},    /* Ч */
 {21,21,21,21,21,21,31}, /* Ш */
 {21,21,21,21,21,31,1},  /* Щ */
 {24,8,8,14,9,9,14},     /* Ъ */
 {17,17,17,29,21,21,29}, /* Ы */
 {16,16,16,30,17,17,30}, /* Ь */
 {14,17,1,7,1,17,14},    /* Э */
 {21,21,29,21,21,21,29}, /* Ю */
 {15,17,17,15,5,9,17}    /* Я */
};
static const u8 yo[7]={10,0,31,16,30,16,31};

static u32 utf8_next(const char **ps){
    const u8 *s=(const u8*)*ps;u32 c=*s++;
    if(c<0x80){*ps=(const char*)s;return c;}
    if((c&0xE0)==0xC0&&s[0]){u32 r=((c&0x1F)<<6)|(s[0]&0x3F);s++;*ps=(const char*)s;return r;}
    if((c&0xF0)==0xE0&&s[0]&&s[1]){u32 r=((c&0x0F)<<12)|((s[0]&0x3F)<<6)|(s[1]&0x3F);s+=2;*ps=(const char*)s;return r;}
    *ps=(const char*)s;return '?';
}
static u8 glyph_row_cp(u32 cp,int row){
    if(row<0||row>6)return 0;
    if(cp>='a'&&cp<='z')cp=cp-'a'+'A';
    if(cp>=0x0430&&cp<=0x044F)cp-=0x20;
    if(cp==0x0451)cp=0x0401;
    if(cp>='A'&&cp<='Z')return latin[cp-'A'][row];
    if(cp>='0'&&cp<='9')return digits[cp-'0'][row];
    if(cp>=0x0410&&cp<=0x042F)return cyr[cp-0x0410][row];
    if(cp==0x0401)return yo[row];
    switch(cp){
...
static char text_fallback(u32 cp){
    if(cp<128)return (char)cp;
    if(cp>=0x0430&&cp<=0x044F)cp-=0x20;
    if(cp==0x0401||cp==0x0451)return 'E';
    if(cp<0x0410||cp>0x042F)return '?';
    static const char map[32]={'A','B','B','G','D','E','X','3','N','N','K','L','M','H','O','P','P','C','T','Y','O','X','U','4','W','W','b','Y','b','E','U','R'};
    return map[cp-0x0410];
}
...
void vga_init(u32 magic,u32 mbi_addr){
    /* Hide the hardware text cursor. PCOS renders its own HUD selection marker. */
    outb(0x3D4,0x0A);outb(0x3D5,(u8)(inb(0x3D5)|0x20));
...
void vga_fill_px(int x,int y,int w,int h,u32 rgb){
    if(!fb_on||w<=0||h<=0)return;if(x<0){w+=x;x=0;}if(y<0){h+=y;y=0;}if(x+w>(int)fb_w)w=(int)fb_w-x;if(y+h>(int)fb_h)h=(int)fb_h-y;if(w<=0||h<=0)return;
    u32 p=pack_rgb(rgb),bytes=(fb_bpp+7)/8;for(int yy=0;yy<h;yy++){u8*d=fb+(u32)(y+yy)*fb_pitch+(u32)x*bytes;for(int xx=0;xx<w;xx++,d+=bytes){if(fb_bpp==32)*(u32*)d=p;else if(fb_bpp==24){d[0]=(u8)p;d[1]=(u8)(p>>8);d[2]=(u8)(p>>16);}else if(fb_bpp==16)*(u16*)d=(u16)p;}}
}
...
static void vga_put_cp(int x,int y,u32 cp,u8 attr){
    if(x<0||y<0||x>=VGA_W||y>=VGA_H)return;
    if(!fb_on){char c=text_fallback(cp);VGA[y*VGA_W+x]=(u16)(u8)c|((u16)attr<<8);return;}
    u32 fg=(attr&15)==vga_hi()?vga_rgb_hi():((attr&15)==vga_dim()?vga_rgb_dim():vga_rgb_fg()),bg=vga_rgb_bg();
    int px=off_x+x*6*scale,py=off_y+y*8*scale;vga_fill_px(px,py,6*scale,8*scale,bg);
    for(int r=0;r<7;r++){u8 bits=glyph_row_cp(cp,r);for(int col=0;col<5;col++)if(bits&(1u<<(4-col)))vga_fill_px(px+col*scale,py+r*scale,scale,scale,fg);}
}
void vga_put(int x,int y,char c,u8 attr){vga_put_cp(x,y,(u8)c,attr);}
...
void vga_text(int x,int y,const char*s,u8 a){while(*s&&x<VGA_W){u32 cp=utf8_next(&s);vga_put_cp(x++,y,cp,a);}}
void vga_text_clip(int x,int y,const char*s,int max,u8 a){int n=0;while(*s&&x<VGA_W&&n<max){u32 cp=utf8_next(&s);vga_put_cp(x++,y,cp,a);n++;}}
