...
static void set_gate(int n, void (*fn)(void), u16 cs){
    u32 a=(u32)fn; idt[n].off_lo=(u16)a; idt[n].sel=cs; idt[n].zero=0;
    idt[n].flags=0x8E; idt[n].off_hi=(u16)(a>>16);
}
...
__attribute__((noreturn)) void fault_handler(u32 vector,u32 error){
    ...
    text("PCOS // FATAL CPU EXCEPTION",0,0x4F); text("VECTOR:",2,0x0F);text(a,3,0x0F);text("ERROR:",5,0x0F);text(b,6,0x0F);text("SYSTEM HALTED - NO AUTOMATIC REBOOT",8,0x0F);
