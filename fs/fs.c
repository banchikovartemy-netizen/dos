...
int fs_exists(const char*path){Overlay*e=ov_find(path);if(e)return e->type!=OV_WHITEOUT;const u8*d;u32 s;if(tar_find(path,&d,&s))return 1;int n=0;fs_list(path,&n);return n>0;}
...
const char*fs_type_name(FsType t){switch(t){case FS_DIR:return"ПАПКА";case FS_FILE:return"ФАЙЛ";case FS_APP:return"ПРИЛ";case FS_DOS:return"DOS";case FS_IMAGE:return"ФОТО";case FS_AUDIO:return"АУДИО";case FS_VIDEO:return"ВИДЕО";default:return"?";}}
